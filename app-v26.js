...
(function() {
    ...
    function openAchievementMembers(id) {
        if (currentUser.role !== 'admin') { showToast('Apenas ADMIN pode gerenciar o progresso das conquistas', 'error'); return; }
    ...
    if (btnSaveAchievementMembers) btnSaveAchievementMembers.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!isTenure) {
                for (const row of rows) {
                    ...
                    if(value!==original) {
                        if(window.AsgardCloud?.setAchievementStage) await window.AsgardCloud.setAchievementStage(managingAchievementId,userId,value,target);
                        ...
                        records.push({id:`stage_${managingAchievementId}_${userId}`,achievementId:String(managingAchievementId),userId,stageOverride:true,stageValue:value,target,updatedAt:new Date().toISOString(),updatedBy:String(currentUser.id)});
            ...
            if (window.AsgardCloud?.setAchievementRecipients) {
                ...
                a.completedBy=selected;a.updatedAt=new Date().toISOString();setStore(DB_ACHIEVEMENTS,achievements);
            } else { a.completedBy=selected;a.updatedAt=new Date().toISOString();setStore(DB_ACHIEVEMENTS,achievements); }

            const users=getStore(DB_USERS)||[];
            selected.filter(id=>!old.has(id)).forEach(id=>{const u=users.find(x=>String(x.id)===id);if(u)addActivity(`${u.callsign} conquistou a insígnia: ${a.title}`);});
        ...
        } catch(err){console.error(err);showToast(err?.message||'Não foi possível salvar o progresso.','error');}
    ...
    function refreshProducts() {
        ...
        productsList.innerHTML = filtered.map(p => {
            ...
            const adminBtns = currentUser.role === 'admin'
                ...
                : '';
            return `
                <article class="product-card product-card-v26" data-product-id="${escapeHtml(String(p.id))}">
                    ...
                    <div class="product-card-footer product-card-footer-v26">
                        <button class="btn-add-cart" data-id="${p.id}" data-nome="${escapeHtml(p.nome)}" data-preco="${p.preco}"><span>🛒</span> Adicionar</button>
                        ${adminBtns ? `<div class="product-admin-actions-v26">${adminBtns}</div>` : ''}
                    </div>
                </article>
            `;
        }).join('');
