...
import {
  ...
  collection, doc, getDoc, getDocs, setDoc, deleteDoc, writeBatch, updateDoc,
...
// V40 — performance: UI cache is immediate, localStorage persistence is batched.
// Chat/realtime snapshots no longer stringify/write large arrays on every tiny update.
const localPersistTimers = new Map();
const lastMirrorSignatures = new Map();
const legacyWriteTimers = new Map();
const legacyWriteValues = new Map();

let app = null, auth = null, db = null, storage = null, initialized = false, connected = false;
let offlineCacheEnabledV56 = false;
...
function persistLocalCacheSoon(key, serialized) {
  clearTimeout(localPersistTimers.get(key));
  // High-volume feeds get a larger coalescing window; UI cache remains immediate.
  const delay = key === 'asgard_messages' ? 900 : key === 'asgard_activity' ? 600 : 180;
  localPersistTimers.set(key, setTimeout(() => {
    localPersistTimers.delete(key);
...
function mirror(key, value) {
  ...
  // Avoid duplicate sync events/render cycles when Firestore emits identical data.
  let serialized = '';
  try { serialized = JSON.stringify(value); } catch (_) {}
  if (serialized && lastMirrorSignatures.get(key) === serialized) return;
  if (serialized) {
    lastMirrorSignatures.set(key, serialized);
    persistLocalCacheSoon(key, serialized);
  }
  emit(key);
}
function loadLocalCache() {
  ['asgard_users', ...Object.keys(ARRAY_COLLECTIONS), 'asgard_contributions'].forEach(k => {
    try {
      ...
      if (k === 'asgard_users' && Array.isArray(parsed)) parsed=parsed.map(normalizeProfile);
      cache.set(k, parsed);
    } catch (_) {}
  });
}
...
async function currentRole() {
  ...
  const local = String((cache.get('asgard_users') || []).find(u => u?.id === me.uid)?.role || '').toLowerCase();
  if (local) return (sessionRole = local);
  const snap = await getDoc(doc(db, 'profiles', me.uid));
  if (snap.exists()) {
    const role=String(snap.data()?.role || '').toLowerCase();
    if (role) return (sessionRole=role);
  }
  return null;
}
...
async function hydrate() {
  ...
  const jobs = [
    readUsers().catch(err => { reportError(err,{kind:'read',scope:'profiles'}); return null; }),
    ...Object.entries(ARRAY_COLLECTIONS).map(([k,c]) =>
      readCollection(k,c,role).catch(err => { reportError(err,{kind:'read',scope:c,key:k}); return null; })
    ),
    readContributions(role).catch(err => { reportError(err,{kind:'read',scope:'contributions'}); return null; })
  ];
...
function watchCollection(key, colName, role = null) {
  const unsub = onSnapshot(collectionSource(colName, role), snap => {
    ...
    remoteCache.set(key, rows);
    mirror(key, rows);
  }, err => reportError(err,{kind:'listener',scope:colName,key}));
  unsubs.push(unsub);
}
async function setupRealtime(roleHint = null) {
  ...
  const role = normalizeRole(roleHint || await currentRole());
  sessionRole = role;
  unsubs.push(onSnapshot(collection(db, 'profiles'), snap => {
    const rows = snap.docs.map(d => normalizeProfile({ id:d.id, ...d.data() }))
      .sort((a,b) => String(a.createdAt || '').localeCompare(String(b.createdAt || '')));
    remoteCache.set('asgard_users', rows);
    mirror('asgard_users', rows);
  }, err => reportError(err,{kind:'listener',scope:'profiles'})));
  ...
  unsubs.push(onSnapshot(doc(db, 'settings', 'contributions'), snap => {
    ...
    mirrorContributions();
  }, err => reportError(err,{kind:'listener',scope:'settings/contributions'})));
  unsubs.push(onSnapshot(contributionSource(role), snap => {
    ...
    contributionMonths = months;
    mirrorContributions();
  }, err => reportError(err,{kind:'listener',scope:'contributions'})));
}
function reportError(err, meta = {}) {
  const code=String(err?.code || '');
  const message=String(err?.message || err || 'Erro Firebase');
  console.error('[Asgard Firebase]', { code, message, meta, error:err });
  window.dispatchEvent(new CustomEvent('asgard:cloud-error', {
    detail:{ code, message, meta, original:err }
  }));
}
...
async function persistArray(key, value) {
  ...
  const rawDesired = Array.isArray(value) ? value : [];
  const oldMap = new Map(current.map((x,i) => [stableId(x,i,key), x]));

  // V41: achievement artwork is persistent media. Some older UI flows still call
  // setStore(DB_ACHIEVEMENTS) with a cached copy of the list. If that copy is stale
  // or temporarily lacks `badge`, merge:false could erase the artwork in Firestore.
  // Preserve the server badge unless a dedicated achievement edit explicitly changes it.
  const desired = key === 'asgard_achievements'
    ? rawDesired.map((item,index) => {
        const id = stableId(item,index,key);
        const remote = oldMap.get(id);
        const localBadge = item?.badge;
        const remoteBadge = remote?.badge;
        if ((!localBadge || String(localBadge).trim() === '') && remoteBadge) {
          return { ...item, badge: remoteBadge };
        }
        return item;
      })
    : rawDesired;
  ...
  desired.forEach((item,index) => {
    ...
    if (!old || JSON.stringify(stripInternal(old)) !== JSON.stringify(stripInternal(item))) {
      const payload=cleanFirestoreObject({ ...stripInternal(item), _index:index });
      // Achievements are merged so unrelated/newer fields (badge, progress map, etc.)
      // cannot be deleted by a stale whole-array write.
      batch.set(doc(db,colName,id), payload, { merge:key === 'asgard_achievements' });
      writes++;
    }
  });
...
function currentContributionMonthV57() {
  const now=new Date();
  return `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
}

async function syncCurrentFinancialStatusesV57() {
  const me=auth?.currentUser;
  if(!me || me.isAnonymous || await currentRole()!=='admin') return false;
  const monthKey=currentContributionMonthV57();
  const [contribSnap,profilesSnap]=await Promise.all([
    getDocs(query(collection(db,'contributions'),where('monthKey','==',monthKey))),
    getDocs(collection(db,'profiles'))
  ]);
  const statuses=new Map();
  contribSnap.forEach(d=>{
    const x=d.data()||{};
    if(x.userId) statuses.set(String(x.userId),String(x.status||'Pendente'));
  });
  const batch=writeBatch(db);
  let writes=0;
  profilesSnap.forEach(d=>{
    ...
    const expected=statuses.get(d.id)||'Pendente';
    const current=String(profile.financialStatus||'Pendente');
    if(current===expected) return;
    batch.set(d.ref,{financialStatus:expected,financialStatusUpdatedAt:new Date().toISOString()},{merge:true});
    writes++;
  });
  if(writes) await batch.commit();
  return true;
...
async function setGameFinanceOverrideV57(gameId,userId,allowed=true) {
  const me=auth?.currentUser;
  if(!me) throw new Error('Sessão expirada.');
  if(await currentRole()!=='admin') throw new Error('Somente o ADMIN pode autorizar presença com pendência financeira.');
  const gid=String(gameId||''),uid=String(userId||'');
  if(!gid||!uid) throw new Error('Operação ou operador inválido.');
  const gameRef=doc(db,'games',gid);
  const profileRef=doc(db,'profiles',uid);
  return await runTransaction(db,async tx=>{
    const [gameSnap,profileSnap]=await Promise.all([tx.get(gameRef),tx.get(profileRef)]);
    if(!gameSnap.exists()) throw new Error('Jogo não encontrado.');
    if(!profileSnap.exists()) throw new Error('Operador não encontrado.');
    const game=gameSnap.data()||{}, profile=profileSnap.data()||{};
    const overrides={...(game.financeOverrides||{})};
    const now=new Date().toISOString();

    if(allowed){
      overrides[uid]={allowed:true,by:me.uid,at:now};
      tx.update(gameRef,{financeOverrides:overrides,updatedAt:now});
      return {allowed:true,game:{...game,financeOverrides:overrides,updatedAt:now}};
    }

    delete overrides[uid];
    const patch={financeOverrides:overrides,updatedAt:now};
    if(String(profile.financialStatus||'Pendente')==='Em Atraso'){
      patch.confirmed=(Array.isArray(game.confirmed)?game.confirmed:[]).filter(x=>String(x)!==uid);
      patch.checkedIn=(Array.isArray(game.checkedIn)?game.checkedIn:[]).filter(x=>String(x)!==uid);
      if(game.loadouts && typeof game.loadouts==='object'){
        const loadouts={...game.loadouts}; delete loadouts[uid]; patch.loadouts=loadouts;
      }
    }
    tx.update(gameRef,patch);
    return {allowed:false,game:{...game,...patch}};
  }).then(result=>{
    if(result?.game) patchGameCacheV58(gid,()=>result.game);
...
async function updateContribution(userId, monthKey, patch) {
  ...
  if (role === 'admin' && String(monthKey) === currentContributionMonthV57() && Object.prototype.hasOwnProperty.call(patch || {}, 'status')) {
    await setDoc(doc(db,'profiles',String(userId)), {
      financialStatus:String(payload.status || 'Pendente'),
      financialStatusUpdatedAt:new Date().toISOString()
    }, { merge:true });
  }
...
async function init() {
  ...
  try {
    ...
    offlineCacheEnabledV56 = true;
  } catch (e) {
    console.warn('[V56] Cache persistente do Firestore indisponível; usando modo padrão.', e?.code || e);
    db = getFirestore(app);
    offlineCacheEnabledV56 = false;
  }
...
function getOfflineStatusV56() {
  return {
    persistentCache: offlineCacheEnabledV56,
...
async function connectSession() {
  ...
  sessionRole = normalizeRole(profile?.role);
...
async function markAnnouncementRead(announcementId) {
  const me=auth?.currentUser; if(!me || me.isAnonymous) throw new Error('Somente operadores podem confirmar leitura.');
  const id=String(announcementId||''); if(!id) throw new Error('Aviso inválido.');
  const payload={announcementId:id,userId:me.uid,readAt:new Date().toISOString()};
  await setDoc(doc(db,'announcement_reads',`${id}_${me.uid}`),payload,{merge:true}); return payload;
}
async function watchAnnouncementReads(cb) {
  const me=auth?.currentUser; if(!me || me.isAnonymous){cb([]);return ()=>{};}
  const role=await currentRole();
  const qref=role==='admin'?collection(db,'announcement_reads'):query(collection(db,'announcement_reads'),where('userId','==',me.uid));
  return onSnapshot(qref,s=>cb(s.docs.map(d=>({id:d.id,...d.data()}))),e=>reportError(e,{kind:'listener',scope:'announcement_reads'}));
}
...
async function createAchievement(achievement) {
  ...
  const current=cache.get('asgard_achievements')||[];
  if(!current.some(a=>String(a.id)===id)) mirror('asgard_achievements',[...current,payload]);
  if (!connected) await readCollection('asgard_achievements', 'achievements', role);
  return payload;
}

function patchAchievementCache(achievementId, patch) {
  const id=String(achievementId||'');
  if(!id) return;
  const current=(cache.get('asgard_achievements')||[]).map(a =>
    String(a.id)===id ? { ...a, ...patch } : a
  );
  remoteCache.set('asgard_achievements',
    (remoteCache.get('asgard_achievements')||current).map(a =>
      String(a.id)===id ? { ...a, ...patch } : a
    )
  );
  mirror('asgard_achievements', current);
}
...
async function updateAchievement(achievementId, patch) {
  ...
  patchAchievementCache(id, payload);
  if (!connected) await readCollection('asgard_achievements', 'achievements', role);
  return true;
}
...
async function setAchievementRecipients(achievementId, userIds) {
  ...
  patchAchievementCache(id,{completedBy,updatedAt:now});
...
function patchGameCacheV58(gameId, updater) {
  const id=String(gameId||'');
  const current=Array.isArray(remoteCache.get('asgard_games'))
    ? remoteCache.get('asgard_games')
    : (Array.isArray(cache.get('asgard_games')) ? cache.get('asgard_games') : []);
  const index=current.findIndex(g=>String(g?.id||'')===id);
  let next=[...current];
  ...
  remoteCache.set('asgard_games',next);
  mirror('asgard_games',next);
  return next;
}
...
async function createGame(game) {
  ...
  const briefingLimits = {
    briefingObjective: 1200,
    briefingRules: 1800,
    briefingMeetingPoint: 180,
    briefingMeetingTime: 10,
    briefingEquipment: 1200,
    briefingNotes: 1200
  };
  for (const [field, limit] of Object.entries(briefingLimits)) {
    if (field in gameData) gameData[field] = String(gameData[field] || '').slice(0, limit);
  }
  if ('briefingImage' in gameData) {
    const image = String(gameData.briefingImage || '');
    if (image && !image.startsWith('data:image/')) throw new Error('Imagem do briefing inválida.');
    if (image.length > 650000) throw new Error('A imagem do briefing é muito grande.');
    gameData.briefingImage = image;
  }
  const payload = cleanFirestoreObject({
    ...gameData,
    id,
  ...
  // V58: do not wait for onSnapshot latency to make a newly-created game visible.
  patchGameCacheV58(id,()=>payload);
  return payload;
}
...
async function updateGame(gameId, patch) {
  ...
  for (const [field, limit] of Object.entries(briefingLimits)) {
    if (field in safe) safe[field] = String(safe[field] || '').slice(0, limit);
  }
  if ('briefingImage' in safe) {
    const image = String(safe.briefingImage || '');
    if (image && !image.startsWith('data:image/')) throw new Error('Imagem do briefing inválida.');
    if (image.length > 650000) throw new Error('A imagem do briefing é muito grande.');
    safe.briefingImage = image;
  }
  ...
  const applied=cleanFirestoreObject({ ...safe, updatedAt:new Date().toISOString() });
  await setDoc(ref, applied, { merge:true });
  patchGameCacheV58(id,existing=>existing?{...existing,...applied}:null);
  return true;
}
...
async function toggleGameConfirmation(gameId, guestName = '') {
  ...
  const [snap,profileSnap] = await Promise.all([getDoc(ref),getDoc(doc(db,'profiles',me.uid))]);
  if (!snap.exists()) throw new Error('Jogo não encontrado.');
  const game = snap.data() || {};
  if (game.completed === true) throw new Error('Esta operação já foi concluída.');
  const confirmed = Array.isArray(game.confirmed) ? game.confirmed : [];
  ...
  if (alreadyConfirmed) {
    ...
    await updateDoc(ref, { confirmed:arrayRemove(me.uid), updatedAt });
    patchGameCacheV58(id,existing=>existing?{...existing,confirmed:(Array.isArray(existing.confirmed)?existing.confirmed:[]).filter(x=>String(x)!==me.uid),updatedAt}:existing);
    return false;
  }
  const financialStatus=String(profileSnap.exists()?(profileSnap.data()?.financialStatus||'Pendente'):'Pendente');
  const override=game.financeOverrides?.[me.uid];
  const allowedByAdmin=override===true || override?.allowed===true;
  if(financialStatus==='Em Atraso' && !allowedByAdmin){
    throw new Error('Sua situação financeira está EM ATRASO. A confirmação de presença depende de autorização do ADMIN.');
  }
  const updatedAt=new Date().toISOString();
  await updateDoc(ref, { confirmed:arrayUnion(me.uid), updatedAt });
  patchGameCacheV58(id,existing=>{
    if(!existing) return existing;
    const confirmed=[...(Array.isArray(existing.confirmed)?existing.confirmed:[])];
    if(!confirmed.some(x=>String(x)===me.uid)) confirmed.push(me.uid);
    return {...existing,confirmed,updatedAt};
  });
  return true;
}

async function writeAuditV55(action, category='system', details={}) {
  const me=auth?.currentUser; if(!me || me.isAnonymous) return false;
  if((await currentRole())!=='admin') return false;
  const profile=await currentProfile();
  const cleanDetails={};
  Object.entries(details||{}).slice(0,12).forEach(([k,v])=>{cleanDetails[String(k).slice(0,50)]=String(v??'').slice(0,500);});
  await addDoc(collection(db,'audit_logs'),{
    action:String(action||'Ação administrativa').slice(0,160),
    category:String(category||'system').slice(0,40),
    details:cleanDetails,
    actorId:me.uid,
    actorCallsign:profile?.callsign||'ADMIN',
    createdAt:new Date().toISOString()
  });
  return true;
}
function watchAuditLogsV55(callback){
  if(!auth?.currentUser) return ()=>{};
  const q=query(collection(db,'audit_logs'),orderBy('createdAt','desc'),limit(200));
  return onSnapshot(q,s=>callback(s.docs.map(d=>({id:d.id,...d.data()}))),e=>console.warn('[audit]',e?.code||e));
}

async function createOrderWithStockV52(order={}) {
  const me=auth?.currentUser;
  if(!me || me.isAnonymous) throw new Error('Somente operadores podem realizar pedidos.');
  const items=Array.isArray(order.items)?order.items:[];
  if(!items.length) throw new Error('Carrinho vazio.');
  const orderRef=doc(db,'orders',String(order.id||crypto.randomUUID()));
  await runTransaction(db,async tx=>{
    const rows=[];
    for(const item of items){
      const ref=doc(db,'products',String(item.productId||''));
      const snap=await tx.get(ref);
      if(!snap.exists()) throw new Error(`${item.nome||'Produto'} não está mais disponível.`);
      const data=snap.data()||{};
      const stock=Math.max(0,Math.floor(Number(data.stock||0)));
      const qty=Math.max(1,Math.floor(Number(item.qtd||1)));
      if(stock<qty) throw new Error(`${data.nome||item.nome||'Produto'} possui apenas ${stock} unidade(s) disponível(is).`);
      rows.push({ref,stock,qty});
    }
    const now=new Date().toISOString();
    rows.forEach(x=>tx.update(x.ref,{stock:x.stock-x.qty,updatedAt:now}));
    tx.set(orderRef,{...order,id:orderRef.id,compradorId:me.uid,status:'Pendente',createdAt:order.createdAt||now,updatedAt:now});
  });
  return orderRef.id;
}

async function updateOrderStatusV51(orderId, newStatus, historyEntry={}) {
  const me=auth?.currentUser; if(!me || me.isAnonymous) throw new Error('Acesso negado.');
  if((await currentRole())!=='admin') throw new Error('Somente ADMIN pode atualizar pedidos.');
  const allowed=['Pendente','Confirmado','Enviado','Entregue','Cancelado'];
  if(!allowed.includes(newStatus)) throw new Error('Status de pedido inválido.');
  const ref=doc(db,'orders',String(orderId||''));
  await runTransaction(db,async tx=>{
    const snap=await tx.get(ref); if(!snap.exists()) throw new Error('Pedido não encontrado.');
    const data=snap.data()||{}, now=new Date().toISOString();
    const history=[...(data.statusHistory||[])];
    history.push({status:newStatus,at:historyEntry.at||now,by:String(historyEntry.by||'ADMIN').slice(0,80)});
    tx.update(ref,{status:newStatus,statusHistory:history.slice(-30),updatedAt:now});
  });
  return true;
}

async function setPostOperationReport(gameId, report={}) {
  const me=auth?.currentUser;if(!me||me.isAnonymous)throw new Error('Acesso negado.');
  if((await currentRole())!=='admin')throw new Error('Somente ADMIN pode registrar o relatório.');
  const profile=await currentProfile();
  const clean={result:String(report.result||'Concluída').slice(0,40),summary:String(report.summary||'').slice(0,1800),positives:String(report.positives||'').slice(0,1200),improvements:String(report.improvements||'').slice(0,1200),notes:String(report.notes||'').slice(0,1200),authorId:me.uid,authorCallsign:profile?.callsign||'Comando',updatedAt:new Date().toISOString()};
  await updateDoc(doc(db,'games',String(gameId)),{postOperationReport:clean,updatedAt:new Date().toISOString()});return clean;
}

async function setGameLoadout(gameId, loadout={}) {
  const me=auth?.currentUser; if(!me || me.isAnonymous) throw new Error('Somente operadores podem selecionar equipamentos.');
  const ref=doc(db,'games',String(gameId||''));
  await runTransaction(db,async tx=>{
    const snap=await tx.get(ref); if(!snap.exists()) throw new Error('Operação não encontrada.');
    const data=snap.data()||{}; if(!(data.confirmed||[]).map(String).includes(me.uid)) throw new Error('Confirme presença antes de selecionar seu equipamento.');
    const loadouts={...(data.loadouts||{})};
    loadouts[me.uid]={primaryId:String(loadout.primaryId||''),secondaryId:String(loadout.secondaryId||''),updatedAt:new Date().toISOString()};
    tx.update(ref,{loadouts,updatedAt:new Date().toISOString()});
  }); return true;
}

async function removeGuestFromGame(gameId, guestUserId) {
  const me = auth?.currentUser;
  if (!me) throw new Error('Sessão expirada.');
  if (await currentRole() !== 'admin') throw new Error('Somente o ADMIN pode remover convidados.');
  const id=String(gameId||'');
  const uid=String(guestUserId||'');
  if(!id || !uid) throw new Error('Convidado ou jogo inválido.');

  const gameRef=doc(db,'games',id);
  const guestRef=doc(db,'guest_confirmations',`${id}_${uid}`);
  return await runTransaction(db, async tx => {
    const [gameSnap,guestSnap]=await Promise.all([tx.get(gameRef),tx.get(guestRef)]);
    if(!gameSnap.exists()) throw new Error('Jogo não encontrado.');
    if(!guestSnap.exists()) throw new Error('Esta confirmação de convidado não existe mais.');
    const guest=guestSnap.data()||{};
    if(String(guest.userId||'')!==uid || String(guest.gameId||'')!==id) throw new Error('Confirmação de convidado inválida.');

    const game=gameSnap.data()||{};
    const confirmed=(Array.isArray(game.confirmed)?game.confirmed:[]).filter(x=>String(x)!==uid);
    const checkedIn=(Array.isArray(game.checkedIn)?game.checkedIn:[]).filter(x=>String(x)!==uid);
    tx.update(gameRef,{confirmed,checkedIn,updatedAt:new Date().toISOString()});
    tx.delete(guestRef);
    return {removed:true,name:String(guest.name||'Convidado')};
  });
}
...
async function setGameCheckin(gameId, userId) {
  ...
  return await runTransaction(db, async tx => {
    ...
    const updatedAt=new Date().toISOString();
    tx.update(ref, { checkedIn, updatedAt });
    return {checkedNow,checkedIn,updatedAt};
  }).then(result=>{
    patchGameCacheV58(String(gameId||''),existing=>existing?{...existing,checkedIn:result.checkedIn,updatedAt:result.updatedAt}:existing);
    return result.checkedNow;
  });
}
...
async function setGameCompleted(gameId, completed = true) {
  ...
  await setDoc(doc(db,'games',id),patch,{merge:true});
  patchGameCacheV58(id,existing=>existing?{...existing,...patch}:existing);
  return true;
}
...
async function setAchievementStage(achievementId, userId, stageValue, target) {
  ...
  const currentAchievement=snap.data()||{};
  const progressByUser={...(currentAchievement.progressByUser||{}),[uid]:{
    value,target:max,updatedAt:now,updatedBy:me.uid
  }};
  await setDoc(achievementRef,{progressByUser,updatedAt:now},{merge:true});
  patchAchievementCache(aid,{progressByUser,updatedAt:now});
...
async function removeSession() {
  const pendingKeys=[...legacyWriteTimers.keys()];
  pendingKeys.forEach(key => clearTimeout(legacyWriteTimers.get(key)));

  // Give pending user actions a short chance to finish before sign-out.
  // Timeout prevents logout from hanging on a poor/offline connection.
  if (pendingKeys.length) {
    const flushes=pendingKeys.map(key => flushLegacyWrite(key).catch(()=>null));
    await Promise.race([
  ...
  localPersistTimers.forEach(t => clearTimeout(t));
  localPersistTimers.clear();
  legacyWriteTimers.clear();
  legacyWriteValues.clear();
}
...
async function flushLegacyWrite(key) {
  legacyWriteTimers.delete(key);
  if (!legacyWriteValues.has(key)) return;
  const value = legacyWriteValues.get(key);
  legacyWriteValues.delete(key);
  ...
  try { await Promise.resolve(task); }
  catch (err) { reportError(err,{kind:'background-write',key}); throw err; }
}

function set(key, value) {
  if (key === 'asgard_session') return;
  // Optimistic UI remains instant. Rapid successive mutations to the same legacy
  // store are coalesced into one Firestore write instead of N duplicate batches.
  mirror(key, value);
  legacyWriteValues.set(key, value);
  clearTimeout(legacyWriteTimers.get(key));
  legacyWriteTimers.set(key, setTimeout(() => { void flushLegacyWrite(key).catch(()=>{}); }, 120));
}

window.AsgardCloud = {
  init, getOfflineStatusV56, connectSession, connectGuestSession, readMyProfile, get, set, hasConfig, removeSession,
  signIn, signInGuest, register, getCurrentUser, waitForAuth, updatePresence, addMessage, uploadChatMedia, updateChatLastRead, updateContribution, syncCurrentFinancialStatusesV57, setGameFinanceOverrideV57, updateUserRole,
  createAnnouncement, updateAnnouncement, removeAnnouncement, markAnnouncementRead, watchAnnouncementReads,
  ...
  appendActivity,
  createGame, updateGame, removeGame, toggleGameConfirmation, removeGuestFromGame, setGameLoadout, setPostOperationReport, updateOrderStatusV51, createOrderWithStockV52, writeAuditV55, watchAuditLogsV55, setGameCheckin, setGameCompleted,
