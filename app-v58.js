...
(function() {
    ...
    async function seedPredefinedAchievementsV21() {
        if (seededAchievementsV21 || String(currentUser?.role||'').toLowerCase() !== 'admin') return;
    ...
    async function finishSplash(targetScreen, afterShow) {
        // V38: depois que a splash inicial termina, novas transições (ex.: LOGIN -> APP)
        // não podem reutilizar a Promise antiga. A Promise única fazia o Firebase
        // autenticar corretamente, mostrar "Bem-vindo", mas a tela permanecia no login.
        const splashAlreadyFinished = splashScreen?.classList.contains('hidden') ||
        ...
        try {
            await splashExitPromise;
        } finally {
            // Libera o controlador para qualquer transição posterior.
            splashExitPromise = null;
        }
    }
    ...
    let announcementReadsV48=[];
    let announcementReadsUnsubV48=null;
    ...
    const gameBriefingObjective = $('game-briefing-objective');
    const gameBriefingRules = $('game-briefing-rules');
    const gameBriefingMeetingPoint = $('game-briefing-meeting-point');
    const gameBriefingMeetingTime = $('game-briefing-meeting-time');
    const gameBriefingEquipment = $('game-briefing-equipment');
    const gameBriefingNotes = $('game-briefing-notes');
    const gameBriefingImageInput = $('game-briefing-image-input');
    const btnSelectGameBriefingImage = $('btn-select-game-briefing-image');
    const btnRemoveGameBriefingImage = $('btn-remove-game-briefing-image');
    const gameBriefingImagePreviewWrap = $('game-briefing-image-preview-wrap');
    const gameBriefingImagePreview = $('game-briefing-image-preview');
    let pendingGameBriefingImage = '';
    ...
    const createProductStock = $('create-product-stock');
    ...
    const editProductStock = $('edit-product-stock');
    ...
    document.addEventListener('visibilitychange', () => {
        ...
        if (document.hidden) {
            ...
            // V40: no UI recomputation while the PWA is in background.
            stopPresenceUiSync();
        } else {
    ...
    btnLogin.addEventListener('click', async () => {
        ...
        try {
            ...
            currentUser = { ...user, role:String(user.role||'operador').toLowerCase(), online:true, lastSeen:new Date().toISOString() };
    ...
    btnRegister.addEventListener('click', async () => {
        ...
        try {
            ...
            currentUser = { ...currentUser, role:String(currentUser.role||'operador').toLowerCase() };
        ...
        try {
            ...
            if (authUser) {
                if (authUser.isAnonymous) {
                    ...
                    if (user) {
                        currentUser = { ...user, role:String(user.role||'operador').toLowerCase() };
                        await pushPresence(true);
                        enterApp();
                        return;
                    }
    ...
    function enterApp() {
        ...
        finishSplash('app-screen', () => {
            // A abertura da tela do app é independente dos renders secundários.
            // Se algum widget falhar, o operador continua dentro do aplicativo.
            try {
                updateUIForRole();
                updateTopbar();
            } catch (uiErr) {
                console.error('[EnterApp UI]', uiErr);
            }
            if (currentUser?.role === 'guest') {
                ...
                if (String(currentUser?.role||'').toLowerCase() === 'admin') setTimeout(seedPredefinedAchievementsV21, 250);
                if (String(currentUser?.role||'').toLowerCase() === 'admin' && window.AsgardCloud?.syncCurrentFinancialStatusesV57) {
                    setTimeout(()=>window.AsgardCloud.syncCurrentFinancialStatusesV57().catch(err=>console.warn('[V58 financeiro]',err?.code||err)),900);
                }
    ...
    function navigateTo(page, forceRefresh = false) {
        ...
        if (page === 'admin' && String(currentUser?.role||'').toLowerCase() !== 'admin') page = 'dashboard';
        ...
        startAnnouncementReadsV48(); refreshDashboardV2(); }
        if (page === 'profile') {
    ...
    function refreshDashboard() {
        ...
        if (announcementAdminBox) {
            announcementAdminBox.classList.toggle('hidden', String(currentUser?.role||'').toLowerCase() !== 'admin');
        }
        if (anns.length > 0) {
            announcements.innerHTML = anns.slice().reverse().map(a => {
                const adminActions = String(currentUser?.role||'').toLowerCase() === 'admin'
                    ...
                    : '';
                const readIds=announcementReadsV48.filter(r=>String(r.announcementId)===String(a.id)).map(r=>String(r.userId));
                const read=readIds.includes(String(currentUser?.id||''));
                const readControls=currentUser?.isGuest?'':`<div class="announcement-read-controls-v48"><button type="button" class="announcement-read-btn-v48 ${read?'done':''}" data-announcement-read="${a.id}" ${read?'disabled':''}>${read?'✓ Leitura confirmada':'Confirmar leitura'}</button>${String(currentUser?.role||'').toLowerCase()==='admin'?`<button type="button" class="announcement-read-report-btn-v48" data-announcement-report="${a.id}">${readIds.length} leitura${readIds.length===1?'':'s'}</button>`:''}</div>`;
    ...
    async function startAnnouncementReadsV48(){
        try{if(announcementReadsUnsubV48){announcementReadsUnsubV48();announcementReadsUnsubV48=null;}
        if(!window.AsgardCloud?.watchAnnouncementReads||currentUser?.isGuest)return;
        announcementReadsUnsubV48=await window.AsgardCloud.watchAnnouncementReads(rows=>{announcementReadsV48=Array.isArray(rows)?rows:[];refreshDashboard();});}
        catch(err){console.warn('[Announcement reads]',err);}
    }
    async function confirmAnnouncementReadV48(id){
        try{await window.AsgardCloud.markAnnouncementRead(id);showToast('Leitura confirmada.','success');}
        catch(err){showToast(err?.message||'Não foi possível confirmar a leitura.','error');}
    }
    function showAnnouncementReadersV48(id){
        if(String(currentUser?.role||'').toLowerCase()!=='admin')return;
        const eligible=(getStore(DB_USERS)||[]).filter(u=>String(u.role||'').toLowerCase()!=='guest');
        const ids=new Set(announcementReadsV48.filter(r=>String(r.announcementId)===String(id)).map(r=>String(r.userId)));
        const read=eligible.filter(u=>ids.has(String(u.id))), pending=eligible.filter(u=>!ids.has(String(u.id)));
        alert(`Confirmaram (${read.length}/${eligible.length}):\n${read.map(u=>'✓ '+(u.callsign||u.name)).join('\n')||'Nenhum'}\n\nPendentes:\n${pending.map(u=>'• '+(u.callsign||u.name)).join('\n')||'Nenhum'}`);
    }
    ...
    announcements?.addEventListener('click', (e) => {
        const readBtn=e.target.closest('[data-announcement-read]');
        if(readBtn){confirmAnnouncementReadV48(readBtn.dataset.announcementRead);return;}
        const reportBtn=e.target.closest('[data-announcement-report]');
        if(reportBtn){showAnnouncementReadersV48(reportBtn.dataset.announcementReport);return;}
    ...
    function openMemberRoleManagerV24(userId) {
        if (String(currentUser?.role||'').toLowerCase() !== 'admin') { showToast('Somente ADMIN pode alterar funções.', 'error'); return; }
        const users=getStore(DB_USERS)||[];
    ...
    btnSaveMemberRole?.addEventListener('click', async () => {
        if(String(currentUser?.role||'').toLowerCase()!=='admin'||!selectedMemberId)return;
    ...
    function refreshAchievements() {
        ...
        const classOrderV36 = [
            { key:'comum', label:'Comuns', rune:'ᚠ', note:'Conquistas de entrada e frequência.' },
            { key:'incomum', label:'Incomuns', rune:'ᚢ', note:'Desafios de consistência e evolução.' },
            { key:'rara', label:'Raras', rune:'ᚱ', note:'Conquistas vinculadas a treinamentos.' },
            { key:'epica', label:'Épicas', rune:'ᛏ', note:'Feitos avançados e operações especiais.' },
            { key:'lendaria', label:'Lendárias', rune:'ᛟ', note:'Marcos máximos de trajetória na equipe.' }
        ];

        const normalizeAchievementClassV36 = value => {
            const normalized=String(value||'comum').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
            if (normalized.includes('lend')) return 'lendaria';
            if (normalized.includes('epic')) return 'epica';
            if (normalized.includes('rar')) return 'rara';
            if (normalized.includes('incom')) return 'incomum';
            return 'comum';
        };

        const renderAchievementCardV36 = a => {
        ...
        achievementsGrid.innerHTML = classOrderV36.map(group => {
            const items=achievements.filter(a => normalizeAchievementClassV36(a.rarity || a.class || a.classe) === group.key);
            if (!items.length) return '';
            return `<section class="achievement-class-quadrant-v36 achievement-class-${group.key}" data-class="${group.key}">
                <header class="achievement-class-header-v36">
                    <span class="achievement-class-rune-v36">${group.rune}</span>
                    <div>
                        <strong>${group.label}</strong>
                        <small>${group.note}</small>
                    </div>
                    <span class="achievement-class-count-v36">${items.length}</span>
                </header>
                <div class="achievement-class-grid-v36">
                    ${items.map(renderAchievementCardV36).join('')}
                </div>
            </section>`;
        }).join('');
    ...
    if (btnSaveAchievementMembers) btnSaveAchievementMembers.addEventListener('click', async () => {
        ...
        try {
            ...
            // V41: when cloud APIs are available, never rewrite the whole achievements
            // array just to update progress. That stale-array rewrite was able to erase
            // a badge selected moments earlier.
    ...
    // V49 — Integração Jogos + Arsenal
    function getArsenalLoadoutV49(userId){
        const uid=String(userId||'');
        const mine=(getStore(DB_ARSENAL)||[]).filter(w=>String(w.userId||w.ownerId||'')===uid);
        return {
            all:mine,
            primary:mine.filter(w=>String(w.type||w.category||'').toLowerCase().includes('prim')),
            secondary:mine.filter(w=>String(w.type||w.category||'').toLowerCase().includes('sec'))
        };
    }
    function weaponNameV49(w){return w?.name||w?.title||w?.model||'Equipamento';}
    function weaponImageV49(w){return w?.image||w?.photo||w?.imageUrl||'';}
    function gameLoadoutV49(game,userId){return game?.loadouts?.[String(userId||'')]||{};}
    async function chooseGameLoadoutV49(gameId){
        const game=(getStore(DB_GAMES)||[]).find(g=>String(g.id)===String(gameId)); if(!game)return;
        if(!(game.confirmed||[]).map(String).includes(String(currentUser.id))){showToast('Confirme presença antes de selecionar seu equipamento.','warning');return;}
        const kit=getArsenalLoadoutV49(currentUser.id);
        if(!kit.all.length){showToast('Cadastre seu equipamento no Arsenal primeiro.','warning');return;}
        const cur=gameLoadoutV49(game,currentUser.id);
        const opts=(arr,val,label)=>[`<option value="">${label}</option>`,...arr.map(w=>`<option value="${w.id}" ${String(val||'')===String(w.id)?'selected':''}>${escapeHtml(weaponNameV49(w))}</option>`)].join('');
        const overlay=document.createElement('div');overlay.className='modal-overlay loadout-overlay-v49';
        overlay.innerHTML=`<div class="loadout-modal-v49"><h3>Equipamento da operação</h3><p>Escolha armas cadastradas no seu Arsenal.</p>
        <label>Arma primária<select id="loadout-primary-v49">${opts(kit.primary,cur.primaryId,'Sem primária')}</select></label>
        <label>Arma secundária<select id="loadout-secondary-v49">${opts(kit.secondary,cur.secondaryId,'Sem secundária')}</select></label>
        <div class="modal-actions"><button class="btn-secondary" id="loadout-cancel-v49">Cancelar</button><button class="btn-primary" id="loadout-save-v49">Salvar</button></div></div>`;
        document.body.appendChild(overlay);
        overlay.querySelector('#loadout-cancel-v49').onclick=()=>overlay.remove();
        overlay.querySelector('#loadout-save-v49').onclick=async()=>{
            try{await AsgardCloud.setGameLoadout(game.id,{primaryId:overlay.querySelector('#loadout-primary-v49').value,secondaryId:overlay.querySelector('#loadout-secondary-v49').value});overlay.remove();showToast('Equipamento vinculado à operação.','success');}
            catch(e){showToast(e?.message||'Não foi possível salvar.','error');}
        };
    }
    function renderGameLoadoutV49(game){
        if(currentUser?.isGuest)return '';
        const kit=getArsenalLoadoutV49(currentUser?.id), sel=gameLoadoutV49(game,currentUser?.id);
        const p=kit.all.find(w=>String(w.id)===String(sel.primaryId||'')), s=kit.all.find(w=>String(w.id)===String(sel.secondaryId||''));
        const item=(w,label)=>w?`<div>${weaponImageV49(w)?`<img src="${weaponImageV49(w)}" alt="">`:''}<span><small>${label}</small><strong>${escapeHtml(weaponNameV49(w))}</strong></span></div>`:'';
        return `<div class="game-loadout-v49"><div class="game-loadout-v49-head"><span>ARSENAL DA OPERAÇÃO</span><button type="button" class="game-loadout-edit-v49" data-game-loadout="${game.id}">${p||s?'Alterar':'Selecionar'}</button></div>${p||s?`<div class="game-loadout-weapons-v49">${item(p,'PRIMÁRIA')}${item(s,'SECUNDÁRIA')}</div>`:'<p>Nenhum equipamento selecionado.</p>'}</div>`;
    }

    // V50 — Relatório Pós-Operação
    function renderPostReportV50(g){
      const r=g?.postOperationReport;
      if(!g?.completed&&!r)return '';
      if(!r)return String(currentUser?.role||'').toLowerCase()==='admin'?`<div class="post-report-v50 empty"><span>RELATÓRIO PÓS-OPERAÇÃO</span><p>Registre o resultado e o debriefing desta operação.</p><button data-post-report="${g.id}">Criar relatório</button></div>`:'';
      return `<details class="post-report-v50"><summary><span>RELATÓRIO PÓS-OPERAÇÃO</span><strong>${escapeHtml(r.result||'Concluída')}</strong><b>⌄</b></summary><div class="post-report-body-v50">
      ${r.summary?`<div><small>RESUMO</small><p>${escapeHtml(r.summary)}</p></div>`:''}
      ${r.positives?`<div><small>PONTOS POSITIVOS</small><p>${escapeHtml(r.positives)}</p></div>`:''}
      ${r.improvements?`<div><small>PONTOS A MELHORAR</small><p>${escapeHtml(r.improvements)}</p></div>`:''}
      ${r.notes?`<div><small>OBSERVAÇÕES</small><p>${escapeHtml(r.notes)}</p></div>`:''}
      <footer>Registrado por ${escapeHtml(r.authorCallsign||'Comando')} ${r.updatedAt?'• '+formatDateTime(r.updatedAt):''}</footer>
      ${String(currentUser?.role||'').toLowerCase()==='admin'?`<button class="post-report-edit-v50" data-post-report="${g.id}">Editar relatório</button>`:''}</div></details>`;
    }
    function openPostReportV50(id){
      if(String(currentUser?.role||'').toLowerCase()!=='admin')return;
      const g=(getStore(DB_GAMES)||[]).find(x=>String(x.id)===String(id));if(!g)return;const r=g.postOperationReport||{};
      const o=document.createElement('div');o.className='modal-overlay post-report-overlay-v50';
      o.innerHTML=`<div class="post-report-modal-v50"><span class="post-report-kicker-v50">DEBRIEFING</span><h3>${escapeHtml(g.title||'Operação')}</h3>
      <label>Resultado<select id="pr-result"><option>Concluída</option><option>Sucesso</option><option>Sucesso parcial</option><option>Falha</option><option>Cancelada</option></select></label>
      <label>Resumo<textarea id="pr-summary" rows="4" maxlength="1800">${escapeHtml(r.summary||'')}</textarea></label>
      <label>Pontos positivos<textarea id="pr-positive" rows="3" maxlength="1200">${escapeHtml(r.positives||'')}</textarea></label>
      <label>Pontos a melhorar<textarea id="pr-improve" rows="3" maxlength="1200">${escapeHtml(r.improvements||'')}</textarea></label>
      <label>Observações<textarea id="pr-notes" rows="3" maxlength="1200">${escapeHtml(r.notes||'')}</textarea></label>
      <div class="modal-actions"><button class="btn-secondary" id="pr-cancel">Cancelar</button><button class="btn-primary" id="pr-save">Salvar relatório</button></div></div>`;
      document.body.appendChild(o);o.querySelector('#pr-result').value=r.result||'Concluída';o.querySelector('#pr-cancel').onclick=()=>o.remove();
      o.querySelector('#pr-save').onclick=async()=>{try{await AsgardCloud.setPostOperationReport(g.id,{result:o.querySelector('#pr-result').value,summary:o.querySelector('#pr-summary').value.trim(),positives:o.querySelector('#pr-positive').value.trim(),improvements:o.querySelector('#pr-improve').value.trim(),notes:o.querySelector('#pr-notes').value.trim()});o.remove();showToast('Relatório pós-operação salvo.','success');}catch(e){showToast(e?.message||'Não foi possível salvar.','error');}};
    }

    // ===== GAMES =====
    function hasGameBriefing(game) {
        return !!(
            String(game?.briefingObjective || '').trim() ||
            String(game?.briefingRules || '').trim() ||
            String(game?.briefingMeetingPoint || '').trim() ||
            String(game?.briefingMeetingTime || '').trim() ||
            String(game?.briefingEquipment || '').trim() ||
            String(game?.briefingNotes || '').trim() ||
            String(game?.briefingImage || '').trim()
        );
    }
    ...
    function setGameBriefingImagePreview(value='') {
        pendingGameBriefingImage = String(value || '');
        if (pendingGameBriefingImage) {
            gameBriefingImagePreview.src = pendingGameBriefingImage;
            gameBriefingImagePreviewWrap.classList.remove('hidden');
            btnRemoveGameBriefingImage.classList.remove('hidden');
        } else {
            gameBriefingImagePreview.removeAttribute('src');
            gameBriefingImagePreviewWrap.classList.add('hidden');
            btnRemoveGameBriefingImage.classList.add('hidden');
        }
    }

    function resetGameBriefingForm(game=null) {
        gameBriefingObjective.value = game?.briefingObjective || '';
        gameBriefingRules.value = game?.briefingRules || '';
        gameBriefingMeetingPoint.value = game?.briefingMeetingPoint || '';
        gameBriefingMeetingTime.value = game?.briefingMeetingTime || '';
        gameBriefingEquipment.value = game?.briefingEquipment || '';
        gameBriefingNotes.value = game?.briefingNotes || '';
        setGameBriefingImagePreview(game?.briefingImage || '');
        if (gameBriefingImageInput) gameBriefingImageInput.value = '';
    }

    // V57 — Bloqueio financeiro para presença em jogos
    function financialStatusV57(userId){
        const data=getContribData();
        const month=data.months?.[getContribMonthKey()]||{};
        return String(month?.[userId]?.status || (getStore(DB_USERS)||[]).find(u=>String(u.id)===String(userId))?.financialStatus || 'Pendente');
    }
    function gameFinanceOverrideV57(game,userId){
        const value=game?.financeOverrides?.[userId];
        return value===true || value?.allowed===true;
    }
    function isFinanciallyBlockedV57(game,userId){
        return financialStatusV57(userId)==='Em Atraso' && !gameFinanceOverrideV57(game,userId);
    }
    function renderFinancialPermissionsV57(game){
        if(String(currentUser?.role||'').toLowerCase()!=='admin' || game.completed) return '';
        const users=(getStore(DB_USERS)||[]).filter(u=>String(u.role||'').toLowerCase()!=='admin' && financialStatusV57(u.id)==='Em Atraso');
        if(!users.length) return '';
        return `<details class="game-finance-v57">
            <summary><span>💳 Permissões financeiras</span><small>${users.length} operador(es) em atraso</small></summary>
            <div class="game-finance-list-v57">
                ${users.map(u=>{
                    const allowed=gameFinanceOverrideV57(game,u.id);
                    return `<div class="game-finance-row-v57">
                        <div><strong>${escapeHtml(u.callsign||u.name||'Operador')}</strong><span>EM ATRASO ${allowed?'• AUTORIZADO NESTE JOGO':'• BLOQUEADO'}</span></div>
                        <button type="button" class="${allowed?'btn-danger':'btn-secondary'} finance-override-v57" data-game="${game.id}" data-user="${u.id}" data-allow="${allowed?'0':'1'}">${allowed?'Revogar permissão':'Autorizar presença'}</button>
                    </div>`;
                }).join('')}
            </div>
        </details>`;
    }
    async function setFinanceOverrideV57(gameId,userId,allow){
        if(String(currentUser?.role||'').toLowerCase()!=='admin') return;
        const users=getStore(DB_USERS)||[];
        const user=users.find(u=>String(u.id)===String(userId));
        const callsign=user?.callsign||user?.name||'Operador';
        const msg=allow
            ? `Autorizar ${callsign}, que está EM ATRASO, a confirmar presença somente neste jogo?`
            : `Revogar a autorização financeira de ${callsign} neste jogo? Se ainda estiver em atraso, ele será removido da lista de presença.`;
        if(!confirm(msg)) return;
        try{
            if(!window.AsgardCloud?.setGameFinanceOverrideV57) throw new Error('Atualize o aplicativo para usar esta função.');
            await window.AsgardCloud.setGameFinanceOverrideV57(gameId,userId,allow);
            if(window.AsgardCloud?.writeAuditV55){
                window.AsgardCloud.writeAuditV55(allow?'Permissão financeira de jogo concedida':'Permissão financeira de jogo revogada','games',{operador:callsign,jogo:String(gameId).slice(0,20)}).catch(()=>{});
            }
            showToast(allow?`${callsign} foi autorizado neste jogo.`:`Autorização de ${callsign} revogada.`,'success');
            refreshGames();
        }catch(err){
            console.error('[Finance override]',err);
            showToast(err?.message||'Não foi possível alterar a permissão.','error');
        }
    }

    function refreshGames() {
        const rawGames = getStore(DB_GAMES);
        const games = Array.isArray(rawGames) ? rawGames.filter(g=>g && typeof g==='object') : [];
        const allUsers = Array.isArray(getStore(DB_USERS)) ? getStore(DB_USERS) : [];
        const filtered = currentFilter === 'all'
            ? games
            : games.filter(g => String(g.type||'').toLowerCase() === String(currentFilter||'').toLowerCase());

        const sorted = [...filtered].sort((a, b) => {
            const ad=new Date(`${a.date||'1970-01-01'}T${a.time||'00:00'}`).getTime();
            const bd=new Date(`${b.date||'1970-01-01'}T${b.time||'00:00'}`).getTime();
        ...
        gamesList.innerHTML = sorted.map(g => {
            const gameType=String(g.type||'jogo').toLowerCase();
            const gameTitle=String(g.title||'Jogo sem título');
            const gameDate=String(g.date||'');
            const gameTime=String(g.time||'');
            const isPast = gameDate ? new Date(`${gameDate}T${gameTime||'23:59'}`) < new Date() : false;
            const confirmedIds = Array.isArray(g.confirmed) ? g.confirmed : [];
            const checkedInIds = Array.isArray(g.checkedIn) ? g.checkedIn : [];
                ...
                return `
                    <div class="confirmed-member ${isCheckedIn ? 'checked-in' : ''}">
                        ...
                        <div class="confirmed-member-status">
                            ...
                            ${isAdmin && isGuestEntry ? `<button class="btn-remove-guest remove-guest-btn" data-game="${g.id}" data-user="${uid}" data-name="${escapeHtml(callsign)}" title="Remover convidado desta operação">Remover</button>` : ''}
                        </div>
                    </div>
                `;
            }).join('');
            ...
            return `
                <div class="game-card ${isPast ? 'game-past-v58' : ''}" data-game-id="${escapeHtml(String(g.id||''))}">
                    <div class="game-card-header">
                        ${g.completed ? '<span class="game-completed-badge">✓ OPERAÇÃO CONCLUÍDA</span>' : ''}
                        <div>
                            <span class="game-type-badge ${escapeHtml(gameType)}">${escapeHtml(gameType.toUpperCase())}</span>
                            <div class="game-title" style="margin-top:6px">${escapeHtml(gameTitle)}</div>
                        </div>
                    </div>
                    <div class="game-meta">
                        <span class="game-meta-item"><span class="meta-icon">📅</span> ${gameDate?formatDate(gameDate):'—'}</span>
                        <span class="game-meta-item"><span class="meta-icon">⏰</span> ${escapeHtml(gameTime || '—')}</span>
                    ...
                    ${hasGameBriefing(g) ? `
                        <details class="game-briefing">
                            <summary>
                                <span class="game-briefing-summary-icon">▣</span>
                                <span>
                                    <strong>Briefing da operação</strong>
                                    <small>Objetivo, regras e informações de apresentação</small>
                                </span>
                                <span class="game-briefing-summary-arrow">⌄</span>
                            </summary>
                            <div class="game-briefing-content">
                                ${g.briefingImage ? `
                                    <button type="button" class="game-briefing-image-button briefing-image-btn" data-id="${g.id}" aria-label="Abrir imagem ou mapa da operação">
                                        <img src="${g.briefingImage}" alt="Mapa ou imagem do briefing de ${escapeHtml(g.title)}" loading="lazy" decoding="async">
                                        <span>Toque para ampliar</span>
                                    </button>
                                ` : ''}
                                <div class="game-briefing-info-grid">
                                    ${g.briefingObjective ? `<div class="game-briefing-block game-briefing-block-wide"><span class="game-briefing-label">OBJETIVO</span><p>${safeMultiline(g.briefingObjective)}</p></div>` : ''}
                                    ${g.briefingMeetingPoint ? `<div class="game-briefing-block"><span class="game-briefing-label">PONTO DE ENCONTRO</span><p>${safeMultiline(g.briefingMeetingPoint)}</p></div>` : ''}
                                    ${g.briefingMeetingTime ? `<div class="game-briefing-block"><span class="game-briefing-label">APRESENTAÇÃO</span><p>${escapeHtml(g.briefingMeetingTime)}</p></div>` : ''}
                                    ${g.briefingRules ? `<div class="game-briefing-block game-briefing-block-wide"><span class="game-briefing-label">REGRAS ESPECÍFICAS</span><p>${safeMultiline(g.briefingRules)}</p></div>` : ''}
                                    ${g.briefingEquipment ? `<div class="game-briefing-block game-briefing-block-wide"><span class="game-briefing-label">EQUIPAMENTOS OBRIGATÓRIOS</span><p>${safeMultiline(g.briefingEquipment)}</p></div>` : ''}
                                    ${g.briefingNotes ? `<div class="game-briefing-block game-briefing-block-wide"><span class="game-briefing-label">OBSERVAÇÕES</span><p>${safeMultiline(g.briefingNotes)}</p></div>` : ''}
                                </div>
                            </div>
                        </details>
                    ` : ''}
                    ${renderGameLoadoutV49(g)}
                    ${renderPostReportV50(g)}
                    ...
                    ` : ''}
                    ${renderFinancialPermissionsV57(g)}
                    <div class="game-actions">
                        ${!g.completed ? `
                            ${(() => {
                                const financeBlocked=currentUser.role!=='guest' && String(currentUser.role||'').toLowerCase()!=='admin' && !isConfirmed && isFinanciallyBlockedV57(g,currentUser.id);
                                const financeAuthorized=currentUser.role!=='guest' && financialStatusV57(currentUser.id)==='Em Atraso' && gameFinanceOverrideV57(g,currentUser.id);
                                return `<button class="btn-secondary confirm-game-btn ${financeBlocked?'finance-blocked-v57':''}" data-id="${g.id}" ${financeBlocked?'disabled':''} ${isConfirmed ? 'style="border-color:var(--success);color:var(--success)"' : ''}>
                                    ${isConfirmed ? '✓ Confirmado' : financeBlocked ? '🔒 Bloqueado — Financeiro' : 'Confirmar Presença'}
                                </button>${financeAuthorized?'<small class="finance-authorized-v57">✓ Presença autorizada pelo ADMIN</small>':''}`;
                            })()}
                        ` : ''}
        ...
        gamesList.querySelectorAll('.remove-guest-btn').forEach(btn => {
            btn.addEventListener('click', () => removeGuestFromGameV44(btn.dataset.game, btn.dataset.user, btn.dataset.name));
        });

        gamesList.querySelectorAll('.finance-override-v57').forEach(btn => {
            btn.addEventListener('click', () => setFinanceOverrideV57(btn.dataset.game, btn.dataset.user, btn.dataset.allow === '1'));
        });

        gamesList.querySelectorAll('[data-post-report]').forEach(btn=>btn.addEventListener('click',()=>openPostReportV50(btn.dataset.postReport)));
        gamesList.querySelectorAll('[data-game-loadout]').forEach(btn=>btn.addEventListener('click',()=>chooseGameLoadoutV49(btn.dataset.gameLoadout)));

        gamesList.querySelectorAll('.briefing-image-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const game = (getStore(DB_GAMES) || []).find(item => String(item.id) === String(btn.dataset.id));
                if (game?.briefingImage) {
                    openPhotoLightbox(game.briefingImage, {
                        title: game.title || 'Briefing da operação',
                        description: 'Mapa / imagem da operação'
                    });
                }
            });
        });
    ...
    async function removeGuestFromGameV44(gameId, userId, name='Convidado') {
        if (String(currentUser?.role||'').toLowerCase() !== 'admin') return;
        const safeName=String(name||'Convidado');
        if (!confirm(`Remover ${safeName} da lista de confirmados desta operação?`)) return;
        try {
            if (!window.AsgardCloud?.removeGuestFromGame) throw new Error('Atualize o aplicativo para usar esta função.');
            await window.AsgardCloud.removeGuestFromGame(gameId,userId);
            showToast(`${safeName} foi removido da operação.`, 'success');
            refreshGames();
        } catch(err) {
            console.error('[Remove guest]',err);
            showToast(err?.message || 'Não foi possível remover o convidado.', 'error');
        }
    }
    ...
    async function toggleConfirmGame(gameId) {
        ...
        if(currentUser.role!=='guest' && String(currentUser.role||'').toLowerCase()!=='admin' && !(game.confirmed||[]).includes(currentUser.id) && isFinanciallyBlockedV57(game,currentUser.id)){
            showToast('Seu financeiro está EM ATRASO. Somente o ADMIN pode liberar sua presença neste jogo.','error');
            return;
        }
        try {
    ...
    btnSelectGameBriefingImage?.addEventListener('click', () => gameBriefingImageInput?.click());

    gameBriefingImageInput?.addEventListener('change', (e) => {
        const file = e.target.files?.[0];
        if (!file) return;
        ...
        if (file.size > 12 * 1024 * 1024) {
            showToast('A imagem original deve ter no máximo 12 MB.', 'error');
            e.target.value = '';
            return;
        }
        resizeImage(file, 900, 0.68, (dataUrl) => {
            if (String(dataUrl || '').length > 650000) {
                showToast('A imagem ainda ficou muito grande. Escolha uma imagem mais leve.', 'error');
                e.target.value = '';
                return;
            }
            setGameBriefingImagePreview(dataUrl);
            e.target.value = '';
            showToast('Imagem do briefing preparada.', 'success');
        });
    });

    btnRemoveGameBriefingImage?.addEventListener('click', () => {
        setGameBriefingImagePreview('');
        if (gameBriefingImageInput) gameBriefingImageInput.value = '';
    });
    ...
    btnSaveGame.addEventListener('click', async () => {
        ...
        const briefingObjective = gameBriefingObjective.value.trim();
        const briefingRules = gameBriefingRules.value.trim();
        const briefingMeetingPoint = gameBriefingMeetingPoint.value.trim();
        const briefingMeetingTime = gameBriefingMeetingTime.value;
        const briefingEquipment = gameBriefingEquipment.value.trim();
        const briefingNotes = gameBriefingNotes.value.trim();
        const briefingImage = pendingGameBriefingImage || '';
        ...
        try {
            if (editingGameId) {
                ...
                if (!game) { showToast('Jogo não encontrado', 'error'); return; }
                const patch = {
                    type, title, date, time, location, description,
                    briefingObjective, briefingRules, briefingMeetingPoint,
                    briefingMeetingTime, briefingEquipment, briefingNotes, briefingImage
                };
                ...
                const game = {
                    ...
                    briefingObjective, briefingRules, briefingMeetingPoint, briefingMeetingTime,
                    briefingEquipment, briefingNotes, briefingImage,
                    createdBy: currentUser.id, confirmed: [], checkedIn: [], completed:false,
                    createdAt: new Date().toISOString()
                };
                if (window.AsgardCloud?.createGame) {
                    const saved=await window.AsgardCloud.createGame(game);
                    const current=Array.isArray(getStore(DB_GAMES))?getStore(DB_GAMES):[];
                    if(saved && !current.some(x=>String(x.id)===String(saved.id))){
                        setStore(DB_GAMES,[...current,saved]);
                    }
                } else {
                    const games=Array.isArray(getStore(DB_GAMES))?getStore(DB_GAMES):[];
                    games.push(game); setStore(DB_GAMES,games);
                }
                addActivity(`${currentUser.callsign} criou ${type}: ${title}`);
                showToast('Jogo criado e sincronizado com sucesso!', 'success');
            }
        } catch (err) {
                ...
                <article class="product-card product-card-v26 product-card-v33" data-product-id="${escapeHtml(String(p.id))}" tabindex="0" role="button" aria-label="Ver ${escapeHtml(p.nome)}">
                    ...
                    <div class="product-info product-info-v26">
                        ...
                        <div class="product-price-v26">
                            ...
                            <span class="product-stock-v52 ${Number(p.stock||0)<=0?'out':Number(p.stock||0)<=3?'low':''}">${Number(p.stock||0)<=0?'Esgotado':Number(p.stock||0)<=3?`Últimas ${Number(p.stock||0)} un.`:`${Number(p.stock||0)} em estoque`}</span>
                        </div>
                    </div>
                    <div class="product-card-footer product-card-footer-v26">
                        <button class="btn-add-cart" data-id="${p.id}" data-nome="${escapeHtml(p.nome)}" data-preco="${p.preco}" ${Number(p.stock||0)<=0?'disabled':''}><span>🛒</span> ${Number(p.stock||0)<=0?'Produto esgotado':'Adicionar ao carrinho'}</button>
                    </div>
                </article>
            `;
        }).join('');
    ...
    async function createProduct() {
        ...
        const stock = Math.max(0, Math.floor(Number(createProductStock?.value || 0)));
        ...
        if (createProductStock) createProductStock.value = '0';
    ...
    function editProduct(productId) {
        ...
        if (editProductStock) editProductStock.value = Math.max(0, Math.floor(Number(product.stock || 0)));
    ...
    async function saveEditProduct() {
        ...
        const stock = Math.max(0, Math.floor(Number(editProductStock?.value || 0)));
        ...
        if (idx < 0) return;
        const patch = { nome, descricao, preco:parseFloat(preco), stock, categoria };
    ...
    function addToCart(productId, nome, preco) {
        const product=(getStore(DB_PRODUCTS)||[]).find(p=>String(p.id)===String(productId));
        const stock=Math.max(0,Math.floor(Number(product?.stock||0)));
        const existing = cart.find(item => item.productId === productId);
        if(stock<=0){showToast('Este produto está esgotado.','warning');return;}
        if(existing && existing.qtd>=stock){showToast(`Limite do estoque atingido (${stock}).`,'warning');return;}
        if (existing) {
            existing.qtd++;
        } else {
    ...
    function updateCartQty(productId, delta) {
        ...
        if(delta>0){
            const product=(getStore(DB_PRODUCTS)||[]).find(p=>String(p.id)===String(productId));
            const stock=Math.max(0,Math.floor(Number(product?.stock||0)));
            if(item.qtd>=stock){showToast(`Limite do estoque atingido (${stock}).`,'warning');return;}
        }
        item.qtd += delta;
        if (item.qtd <= 0) {
    ...
    btnWhatsappCheckout.addEventListener('click', async () => {
        if (cart.length === 0) {
        ...
        const orderData = {
            ...
            status: 'Pendente',
            statusHistory: [{ status: 'Pendente', at: new Date().toISOString(), by: currentUser.callsign || currentUser.name || 'Operador' }],
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
        };

        try {
            await AsgardCloud.createOrderWithStockV52(orderData);
            openWhatsApp(orderData);
            clearCart();
            cartModal.classList.add('hidden');
            showToast('Pedido registrado e estoque atualizado!', 'success');
        } catch (err) {
            console.error(err);
            showToast(err?.message || 'Não foi possível concluir o pedido.', 'error');
            refreshProducts();
        }
    });
    ...
    async function updateOrderStatus(orderId, newStatus) {
        ...
        if (!order) return;
        const previous = order.status || 'Pendente';
        if (previous === newStatus) return;
        const at = new Date().toISOString();
        const entry = { status:newStatus, at, by:currentUser?.callsign || currentUser?.name || 'ADMIN' };
        try {
            if (window.AsgardCloud?.updateOrderStatusV51) {
                await window.AsgardCloud.updateOrderStatusV51(orderId, newStatus, entry);
            } else {
            ...
            addActivity(`Pedido #${orderId.substring(0, 8).toUpperCase()} atualizado de ${previous} para ${newStatus}`);
            showToast(`Status atualizado para ${newStatus}`, 'success');
            refreshOrderStats();
            refreshOrders();
            renderMyOrdersV2();
        } catch (err) {
            showToast(err?.message || 'Não foi possível atualizar o pedido.', 'error');
            refreshOrders();
        }
    }
    ...
    async function toggleAchievementValidationV20(achievementId) {
        ...
        if(count>=target && !already){
            const completedBy=[...new Set([...(a.completedBy||[]).map(String),String(userId)])];
            if(window.AsgardCloud?.setAchievementRecipients){
                try{await window.AsgardCloud.setAchievementRecipients(achievementId,completedBy);}
                catch(err){console.error(err);showToast(err?.message||'Não foi possível concluir a conquista.','error');return;}
            } else {
                a.completedBy=completedBy;a.updatedAt=new Date().toISOString();setStore(DB_ACHIEVEMENTS,achievements);
            }
            const u=(getStore(DB_USERS)||[]).find(x=>String(x.id)===String(userId)); addActivity(`${u?.callsign||'Operador'} conquistou a insígnia: ${a.title}`); playSfx('achievement',.95); showToast(`Conquista desbloqueada: ${a.title}`, 'success');
        }
    ...
    function refreshAdminPanel() {
        ...
        renderCommandOverviewV53();
        updateAdminAlertBadge();
        const active = [...adminTabs].find(b => b.classList.contains('active'))?.dataset.adminTab || 'overview';
        setAdminTab(active);
    }

    function renderCommandOverviewV53(){
        if(String(currentUser?.role||'').toLowerCase()!=='admin') return;
        const users=(getStore(DB_USERS)||[]).filter(u=>String(u.role||'').toLowerCase()!=='guest');
        const games=getStore(DB_GAMES)||[];
        const orders=getStore(DB_ORDERS)||[];
        const products=getStore(DB_PRODUCTS)||[];
        const now=Date.now();

        const future=games.filter(g=>!g.completed && g.date).map(g=>{
            const t=new Date(String(g.date).length<=10?`${g.date}T23:59:59`:g.date).getTime();
            return {g,t};
        }).filter(x=>Number.isFinite(x.t)&&x.t>=now-86400000).sort((a,b)=>a.t-b.t);
        const next=future[0]?.g;
        if($('admin-next-game-title')) $('admin-next-game-title').textContent=next?.title||'Nenhuma operação agendada';
        if($('admin-next-game-details')) $('admin-next-game-details').textContent=next?`${formatDate(next.date)} • ${next.location||'Local não informado'}`:'Crie uma nova operação para iniciar o planejamento.';
        const confirmed=next?(next.confirmed||[]).length:0;
        const checked=next?(next.checkedIn||[]).length:0;
        const loadouts=next?Object.keys(next.loadouts||{}).filter(uid=>(next.confirmed||[]).map(String).includes(String(uid))).length:0;
        const readiness=confirmed?Math.round(((checked+loadouts)/(confirmed*2))*100):0;
        if($('admin-next-confirmed')) $('admin-next-confirmed').textContent=confirmed;
        if($('admin-next-checked')) $('admin-next-checked').textContent=checked;
        if($('admin-next-loadouts')) $('admin-next-loadouts').textContent=loadouts;
        if($('admin-next-readiness')) $('admin-next-readiness').textContent=`${Math.min(100,readiness)}%`;

        const completed=games.filter(g=>g.completed).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))).slice(0,5);
        let totalExpected=0,totalPresent=0;
        completed.forEach(g=>{totalExpected+=(g.confirmed||[]).length;totalPresent+=(g.checkedIn||[]).length;});
        const attendance=totalExpected?Math.round(totalPresent/totalExpected*100):0;
        if($('admin-attendance-rate')) $('admin-attendance-rate').textContent=`${attendance}%`;

        const alertData=getAdminAlertData();
        if($('admin-overdue-count')) $('admin-overdue-count').textContent=alertData.overdue.length;
        const online=users.filter(u=>u.online===true || (u.lastSeen && now-new Date(u.lastSeen).getTime()<6*60*1000)).length;
        if($('admin-online-count')) $('admin-online-count').textContent=online;

        const announcements=getStore(DB_ANNOUNCEMENTS)||[];
        const important=announcements.filter(a=>a.important||a.requiresReadReceipt);
        const unread=important.reduce((sum,a)=>{
            const readIds=new Set((announcementReadsV48||[]).filter(r=>String(r.announcementId)===String(a.id)).map(r=>String(r.userId)));
            return sum+users.filter(u=>String(u.role||'').toLowerCase()!=='admin'&&!readIds.has(String(u.id))).length;
        },0);
        if($('admin-unread-notices')) $('admin-unread-notices').textContent=unread;

        const pending=orders.filter(o=>o.status==='Pendente').length;
        const revenue=orders.filter(o=>o.status==='Entregue').reduce((s,o)=>s+Number(o.total||0),0);
        const low=products.filter(p=>Number(p.stock||0)>0&&Number(p.stock||0)<=3);
        const out=products.filter(p=>Number(p.stock||0)<=0);
        if($('admin-store-pending')) $('admin-store-pending').textContent=pending;
        if($('admin-store-revenue')) $('admin-store-revenue').textContent=`R$ ${revenue.toFixed(2).replace('.',',')}`;
        if($('admin-low-stock')) $('admin-low-stock').textContent=low.length;
        if($('admin-out-stock')) $('admin-out-stock').textContent=out.length;
        if($('admin-low-stock-list')) $('admin-low-stock-list').innerHTML=[...out,...low].slice(0,5).map(p=>`<div><span>${escapeHtml(p.nome||'Produto')}</span><strong class="${Number(p.stock||0)<=0?'out':''}">${Number(p.stock||0)<=0?'ESGOTADO':`${Number(p.stock||0)} un.`}</strong></div>`).join('') || '<small>Estoque em níveis normais.</small>';

        const priority=[];
        if(alertData.overdue.length) priority.push({icon:'💳',text:`${alertData.overdue.length} contribuição(ões) em atraso`});
        if(pending) priority.push({icon:'📦',text:`${pending} pedido(s) aguardando atendimento`});
        if(alertData.needCheckin.length) priority.push({icon:'🎯',text:`${alertData.needCheckin.length} operação(ões) com check-in pendente`});
        if(out.length) priority.push({icon:'⚠️',text:`${out.length} produto(s) sem estoque`});
        if(unread) priority.push({icon:'📢',text:`${unread} confirmação(ões) de aviso pendente(s)`});
        if($('admin-priority-list')) $('admin-priority-list').innerHTML=priority.length?priority.slice(0,5).map(x=>`<div><span>${x.icon}</span><strong>${escapeHtml(x.text)}</strong></div>`).join(''):'<div class="command-priority-ok-v53">✓ Nenhuma pendência crítica encontrada.</div>';
    }
    ...
    $('admin-next-game-action')?.addEventListener('click', () => navigateTo('games'));
    $('admin-store-action')?.addEventListener('click', () => setAdminTab('sales'));
    $('admin-priority-action')?.addEventListener('click', () => setAdminTab('alerts'));
    ...
    let cloudReconnectTimerV43 = null;
    let lastCloudToastAtV43 = 0;
    let lastPermissionBackgroundAtV43 = 0;
    ...
    let profileHistoryFilterV47='all';
    let profileHistoryUserIdV47='';

    function normalizeGameTypeV46(type) {
        const raw = String(type || '').trim().toLowerCase();
        if (raw.includes('trein')) return 'training';
        if (raw.includes('opera')) return 'operation';
        // Jogos antigos normalmente usam "jogo". Para estatística operacional,
        // qualquer partida que não seja treinamento conta como operação.
        return 'operation';
    }

    function formatTeamTenureV46(dateValue) {
        const start = new Date(dateValue || '');
        if (Number.isNaN(start.getTime())) return { label:'—', since:'Data de entrada indisponível' };
        const now = new Date();
        if (start > now) return { label:'0 meses', since:`Membro desde ${formatDate(dateValue)}` };
        ...
        if (months > 0 || years === 0) label += `${label ? ' ' : ''}${months}m`;
        return { label, since:`Membro desde ${formatDate(dateValue)}` };
    }

    function getOperatorStatsV46(userId) {
        const id = String(userId || '');
        const users = getStore(DB_USERS) || [];
        const user = users.find(u => String(u.id || '') === id);
        const games = getStore(DB_GAMES) || [];
        const completed = games.filter(g => !!g.completed);
        const attended = completed.filter(g => (g.checkedIn || []).map(String).includes(id));
        const confirmedCompleted = completed.filter(g => (g.confirmed || []).map(String).includes(id));
        const confirmedAll = games.filter(g => (g.confirmed || []).map(String).includes(id));

        const attendedOperations = attended.filter(g => normalizeGameTypeV46(g.type) === 'operation');
        const attendedTrainings = attended.filter(g => normalizeGameTypeV46(g.type) === 'training');

        // Uma conquista pode existir tanto no catálogo legado (completedBy)
        // quanto em achievement_awards. Usar Set evita contagem duplicada.
        const awardedIds = new Set(
            (getStore(DB_ACHIEVEMENT_AWARDS) || [])
                .filter(a => String(a.userId || '') === id)
                .map(a => String(a.achievementId || ''))
        );
        const achievementCatalog = getStore(DB_ACHIEVEMENTS) || [];
        achievementCatalog.forEach(a => {
            if ((a.completedBy || []).map(String).includes(id)) awardedIds.add(String(a.id || ''));
        });

        const achievementXp = [...awardedIds].reduce((sum, achievementId) => {
            const achievement = achievementCatalog.find(a => String(a.id || '') === achievementId);
        ...
        // Presença só considera operações já concluídas em que o operador confirmou.
        // Assim, partidas futuras não reduzem artificialmente a taxa.
        const attendedConfirmed = confirmedCompleted.filter(g => (g.checkedIn || []).map(String).includes(id)).length;
        const attendanceRate = confirmedCompleted.length
            ? Math.round((attendedConfirmed / confirmedCompleted.length) * 100)
            : 0;

        const tenure = formatTeamTenureV46(user?.teamJoinedAt || user?.memberSince || user?.createdAt);

        return {
            user, games, completed, attended, confirmedAll, confirmedCompleted,
            attendedOperations, attendedTrainings,
            awardedIds, achievementCatalog,
            xp, level, inLevel, attendanceRate, attendedConfirmed, tenure
        };
    }

    function renderProfileStatsV2(userId) {
        const s = getOperatorStatsV46(userId);

        if ($('profile-level')) $('profile-level').textContent = s.level;
        if ($('profile-xp')) $('profile-xp').textContent = `${s.xp} XP`;
        if ($('profile-xp-next')) {
            const remaining = s.inLevel === 0 && s.xp > 0 ? 250 : 250 - s.inLevel;
            $('profile-xp-next').textContent = `${remaining} XP para o próximo nível`;
        }
        if ($('profile-xp-bar')) $('profile-xp-bar').style.width = `${(s.inLevel / 250) * 100}%`;

        if ($('profile-ops-count')) $('profile-ops-count').textContent = s.attendedOperations.length;
        if ($('profile-ops-detail')) $('profile-ops-detail').textContent =
            `${s.attendedOperations.length} ${s.attendedOperations.length === 1 ? 'concluída' : 'concluídas'}`;

        if ($('profile-training-count')) $('profile-training-count').textContent = s.attendedTrainings.length;
        if ($('profile-training-detail')) $('profile-training-detail').textContent =
            `${s.attendedTrainings.length} ${s.attendedTrainings.length === 1 ? 'concluído' : 'concluídos'}`;

        if ($('profile-attendance-rate')) $('profile-attendance-rate').textContent = `${s.attendanceRate}%`;
        if ($('profile-attendance-detail')) $('profile-attendance-detail').textContent =
            `${s.attendedConfirmed} de ${s.confirmedCompleted.length} confirmados`;

        if ($('profile-badges-count')) $('profile-badges-count').textContent =
            `${s.awardedIds.size}/${s.achievementCatalog.length}`;
        if ($('profile-badges-detail')) $('profile-badges-detail').textContent =
            `${s.awardedIds.size} ${s.awardedIds.size === 1 ? 'desbloqueada' : 'desbloqueadas'}`;

        if ($('profile-team-tenure')) $('profile-team-tenure').textContent = s.tenure.label;
        if ($('profile-team-since-detail')) $('profile-team-since-detail').textContent = s.tenure.since;
        if ($('profile-stats-updated')) $('profile-stats-updated').textContent = 'Tempo real';

        profileHistoryUserIdV47=String(userId||'');
        renderIndividualHistoryV47(profileHistoryUserIdV47);
    }

    function getIndividualHistoryV47(userId){
        const id=String(userId||''), events=[];
        (getStore(DB_GAMES)||[]).forEach(g=>{
            const confirmed=(g.confirmed||[]).map(String).includes(id);
            const checked=(g.checkedIn||[]).map(String).includes(id);
            if(!confirmed&&!checked)return;
            const training=normalizeGameTypeV46(g.type)==='training';
            const status=checked?'Presente':(g.completed?'Ausente':'Confirmado');
            events.push({type:'game',date:g.date||g.createdAt||'',title:g.title||(training?'Treinamento':'Operação'),subtitle:`${training?'Treinamento':'Operação'} • ${status}`,detail:g.location||'Local não informado',status,checked});
        });
        const achievements=getStore(DB_ACHIEVEMENTS)||[];
        const awardMap=new Map((getStore(DB_ACHIEVEMENT_AWARDS)||[]).filter(a=>String(a.userId||'')===id).map(a=>[String(a.achievementId||''),a]));
        achievements.forEach(a=>{
            const award=awardMap.get(String(a.id||'')), legacy=(a.completedBy||[]).map(String).includes(id);
            if(!award&&!legacy)return;
            events.push({type:'achievement',date:award?.awardedAt||award?.createdAt||a.updatedAt||a.createdAt||'',title:a.title||a.name||'Conquista',subtitle:`Conquista • ${String(a.rarity||'Comum')}`,detail:a.description||'Insígnia conquistada',badge:a.badge||''});
        });
        return events.sort((a,b)=>(new Date(b.date||0).getTime()||0)-(new Date(a.date||0).getTime()||0));
    }

    function renderIndividualHistoryV47(userId){
        const hist=$('profile-operation-history'); if(!hist)return;
        const all=getIndividualHistoryV47(userId);
        const filtered=all.filter(e=>profileHistoryFilterV47==='all'||(profileHistoryFilterV47==='games'&&e.type==='game')||(profileHistoryFilterV47==='achievements'&&e.type==='achievement'));
        if($('profile-history-total'))$('profile-history-total').textContent=`${all.length} ${all.length===1?'registro':'registros'}`;
        if(!filtered.length){hist.innerHTML='<p class="empty-state">Nenhum registro nesta categoria.</p>';return;}
        hist.innerHTML=filtered.map(e=>{
            const d=e.date?formatDate(String(e.date).slice(0,10)):'Data não registrada';
            if(e.type==='achievement')return `<div class="operation-history-item history-event-v47 history-achievement-v47"><div class="history-event-icon-v47">${e.badge?`<img src="${e.badge}" alt="" loading="lazy" decoding="async">`:'✦'}</div><div class="history-event-copy-v47"><strong>${escapeHtml(e.title)}</strong><span>${escapeHtml(e.subtitle)}</span><small>${d} • ${escapeHtml(e.detail)}</small></div><span class="history-event-tag-v47">CONQUISTA</span></div>`;
            return `<div class="operation-history-item history-event-v47"><div class="history-event-icon-v47">${e.checked?'✓':'◈'}</div><div class="history-event-copy-v47"><strong>${escapeHtml(e.title)}</strong><span>${escapeHtml(e.subtitle)}</span><small>${d} • ${escapeHtml(e.detail)}</small></div><span class="history-event-tag-v47 ${e.checked?'history-event-tag-present':''}">${escapeHtml(e.status)}</span></div>`;
        }).join('');
    }

    document.querySelectorAll('.profile-history-filter').forEach(btn=>btn.addEventListener('click',()=>{
        profileHistoryFilterV47=btn.dataset.historyFilter||'all';
        document.querySelectorAll('.profile-history-filter').forEach(b=>b.classList.toggle('active',b===btn));
        renderIndividualHistoryV47(profileHistoryUserIdV47||currentUser?.id);
    }));
    ...
    function orderStatusIndexV51(status){
        return {Pendente:0,Confirmado:1,Enviado:2,Entregue:3,Cancelado:-1}[status] ?? 0;
    }
    function renderMyOrdersV2() {
        ...
        if(!orders.length){el.innerHTML='<p class="empty-state">Você ainda não realizou pedidos.</p>';return;}
        const steps=['Pendente','Confirmado','Enviado','Entregue'];
        el.innerHTML=orders.map(o=>{
            const idx=orderStatusIndexV51(o.status);
            const history=(o.statusHistory||[]).slice().sort((a,b)=>parseTimeV2(b.at)-parseTimeV2(a.at));
            return `<article class="my-order-card my-order-card-v51">
              <div class="my-order-head"><div><strong>#${String(o.id||'').slice(0,8).toUpperCase()}</strong><small>${new Date(o.createdAt||Date.now()).toLocaleString('pt-BR')}</small></div><span class="vendas-order-status-badge status-${o.status}">${escapeHtml(o.status||'Pendente')}</span></div>
              ${o.status==='Cancelado'?`<div class="order-cancelled-v51">Pedido cancelado</div>`:`<div class="order-progress-v51">${steps.map((s,i)=>`<div class="${i<=idx?'done':''} ${i===idx?'current':''}"><i>${i<idx?'✓':i+1}</i><span>${s}</span></div>`).join('')}</div>`}
              <div class="my-order-items">${(o.items||o.itens||[]).map(i=>`${escapeHtml(i.nome||'Item')} × ${Number(i.qtd||1)}`).join(' • ')}</div>
              <div class="order-history-v51">${history.length?`<details><summary>Histórico de atualizações</summary>${history.map(x=>`<div><strong>${escapeHtml(x.status||'Atualização')}</strong><span>${x.at?new Date(x.at).toLocaleString('pt-BR'):''}${x.by?' • '+escapeHtml(x.by):''}</span></div>`).join('')}</details>`:''}</div>
              <div class="my-order-foot"><span>Última atualização: ${new Date(o.updatedAt||o.createdAt||Date.now()).toLocaleString('pt-BR')}</span><strong>R$ ${Number(o.total||0).toFixed(2).replace('.',',')}</strong></div>
            </article>`;
        }).join('');
    }
    ...
    // V56 — Modo de operação com internet ruim
    const networkModeV56=$('network-mode-v56');
    const connectionApiV56=navigator.connection || navigator.mozConnection || navigator.webkitConnection;
    let networkModeTimerV56=null;

    function networkProfileV56(){
        if(!navigator.onLine) return {mode:'offline',label:'Offline',quality:'OFFLINE'};
        const type=String(connectionApiV56?.effectiveType||'').toLowerCase();
        const save=Boolean(connectionApiV56?.saveData);
        const slow=save || type==='slow-2g' || type==='2g';
        const unstable=type==='3g';
        return slow
          ? {mode:'slow',label:'Rede lenta',quality:save?'ECONOMIA':'2G'}
          : unstable
            ? {mode:'unstable',label:'Conexão limitada',quality:'3G'}
            : {mode:'online',label:'Online',quality:type?type.toUpperCase():'ONLINE'};
    }
    function updateNetworkModeV56(showToastChange=false){
        const p=networkProfileV56();
        document.body.classList.toggle('low-bandwidth-v56',p.mode==='slow'||p.mode==='unstable');
        document.body.classList.toggle('contingency-offline-v56',p.mode==='offline');
        if(networkModeV56){
            const visible=p.mode!=='online';
            networkModeV56.classList.toggle('hidden',!visible);
            networkModeV56.dataset.mode=p.mode;
            const title=$('network-mode-title-v56'), text=$('network-mode-text-v56'), quality=$('network-mode-quality-v56');
            if(title) title.textContent=p.mode==='offline'?'Modo contingência offline':'Modo economia de dados';
            if(text) text.textContent=p.mode==='offline'
              ? 'Dados já carregados continuam disponíveis. Ações com validação online devem aguardar a reconexão.'
              : 'Conexão limitada detectada. O app reduziu efeitos e prioriza dados essenciais.';
            if(quality) quality.textContent=p.quality;
        }
        if(connectionStatusV2){
            const label=connectionStatusV2.querySelector('.connection-label');
            if(label) label.textContent=p.label;
        }
        if(showToastChange && p.mode==='offline') showToast('Modo contingência ativado — conteúdo salvo continua disponível.','info');
        if(showToastChange && p.mode==='online') showToast('Conexão normalizada. Sincronizando dados...','success');
    }
    connectionApiV56?.addEventListener?.('change',()=>updateNetworkModeV56(false));
    ...
    function updateConnectionUiV2() {
        ...
        clearTimeout(networkModeTimerV56);
        networkModeTimerV56=setTimeout(()=>updateNetworkModeV56(false),0);
    }
    window.addEventListener('online',()=>{ clearTimeout(cloudReconnectTimerV43); cloudHasRecentErrorV2=false; updateConnectionUiV2(); updateNetworkModeV56(true); });
    window.addEventListener('offline',()=>{ updateConnectionUiV2(); updateNetworkModeV56(true); });
    updateConnectionUiV2();
    updateNetworkModeV56(false);
    ...
    window.addEventListener('asgard:sync', (event) => {
        ...
        if (key === DB_USERS) {
            ...
            if (currentUser) currentUser.role=String(currentUser.role||'operador').toLowerCase();
        ...
        if (String(currentUser?.role||'').toLowerCase() === 'admin' && [DB_GAMES,DB_ORDERS,DB_CONTRIBUTIONS,'asgard_guest_confirmations'].includes(key)) updateAdminAlertBadge();
        scheduleSyncedViewRefresh(key);
    });
    window.addEventListener('asgard:cloud-error', (e) => {
        const detail=e.detail || {};
        console.error('[Cloud sync]', detail);
        const code = String(detail.code || detail.message || '').toLowerCase();
        const kind = String(detail.meta?.kind || 'unknown');
        const isPermission = code.includes('permission-denied') || code.includes('insufficient permission');
        const isConnectivity = code.includes('unavailable') || code.includes('deadline-exceeded') || code.includes('network') || !navigator.onLine;
        const isQuota = code.includes('resource-exhausted') || code.includes('quota');
        const now=Date.now();

        // V43: 'Reconectando' representa somente problema de rede/servidor.
        // Erros de permissão não significam perda de conexão e não devem deixar
        // o app piscando entre Online/Reconectando.
        if (isConnectivity) {
            cloudHasRecentErrorV2 = true;
            updateConnectionUiV2();
            clearTimeout(cloudReconnectTimerV43);
            cloudReconnectTimerV43=setTimeout(()=>{
                cloudHasRecentErrorV2=false;
                updateConnectionUiV2();
            }, 5000);
            if (now-lastCloudToastAtV43 > 12000) {
                lastCloudToastAtV43=now;
                showToast('Conexão instável com o servidor. Tentando sincronizar novamente...', 'info');
            }
            return;
        }

        if (isQuota) {
            if (now-lastCloudToastAtV43 > 30000) {
                lastCloudToastAtV43=now;
                showToast('Cota do Firebase temporariamente indisponível. Algumas atualizações podem atrasar.', 'error');
            }
            return;
        }

        if (isPermission) {
            // Leituras/listeners opcionais ou gravações de compatibilidade em segundo
            // plano não devem assustar o usuário com um falso erro de ADMIN ao entrar.
            if (kind === 'listener' || kind === 'read' || kind === 'background-write') {
                if (now-lastPermissionBackgroundAtV43 > 60000) {
                    lastPermissionBackgroundAtV43=now;
                    console.warn('[Asgard] Operação Firebase auxiliar bloqueada pelas regras.', detail.meta);
                }
                return;
            }
            return;
        ...
        if (now-lastCloudToastAtV43 > 15000) {
            lastCloudToastAtV43=now;
            showToast('Uma sincronização em segundo plano não pôde ser concluída.', 'error');
        }
    });
    ...
    // ===== V40 — PERFORMANCE & STABILITY =====
    // Images inside app pages decode asynchronously and load only near viewport.
    // Login/logo assets are intentionally left eager.
    function optimizeAppImagesV40(root=document) {
        root.querySelectorAll?.('.page img').forEach(img => {
            if (!img.hasAttribute('loading')) img.loading='lazy';
            if (!img.hasAttribute('decoding')) img.decoding='async';
        });
    }
    optimizeAppImagesV40();
    const imageObserverV40 = new MutationObserver(mutations => {
        mutations.forEach(m => m.addedNodes.forEach(node => {
            if (!(node instanceof Element)) return;
            if (node.matches?.('.page img')) optimizeAppImagesV40(node.parentElement || node);
            else if (node.querySelector?.('.page img, img')) optimizeAppImagesV40(node);
        }));
    });
    imageObserverV40.observe(document.getElementById('main-content') || document.body,{childList:true,subtree:true});

    // Prevent accidental double submission of destructive/save/confirm actions.
    // Navigation, cart quantity and ordinary toggles are excluded.
    const lastActionAtV40 = new WeakMap();
    document.addEventListener('click', e => {
        const btn=e.target.closest('button');
        if(!btn || btn.disabled) return;
        const cls=String(btn.className||'');
        const id=String(btn.id||'');
        const guarded=/save|salvar|confirm|delete|excluir|remove|create|criar|checkout|finaliz|promote|role/i.test(`${cls} ${id} ${btn.textContent||''}`);
        if(!guarded) return;
        const now=performance.now();
        const last=lastActionAtV40.get(btn)||0;
        if(now-last < 650){
            e.preventDefault();
            e.stopImmediatePropagation();
            return;
        }
        lastActionAtV40.set(btn,now);
    },true);

    // Errors in one visual refresh should never cascade into another module.
    window.addEventListener('error', event => {
        if (!event?.error) return;
        console.warn('[Asgard V40 UI isolation]', event.error);
    });
    ...
    // ===== V37 — VISUAL SYSTEM (UI ONLY) =====
    const visualPageMetaV37 = {
        dashboard:['CENTRAL','Visão geral do operador e da equipe'],
        profile:['OPERADOR','Identidade, conquistas e personalização'],
        members:['EQUIPE','Operadores e funções do esquadrão'],
        arsenal:['EQUIPAMENTO','Armas primárias e secundárias da equipe'],
        achievements:['HONRARIAS','Progresso, classes e insígnias'],
        chat:['COMUNICAÇÕES','Canal operacional da equipe'],
        games:['MISSÕES','Operações abertas e histórico'],
        contribuicao:['LOGÍSTICA','Contribuições mensais da equipe'],
        configuracoes:['SISTEMA','Preferências e instalação do aplicativo'],
        loja:['SUPRIMENTOS','Produtos e equipamentos disponíveis'],
        admin:['COMANDO','Gestão administrativa da equipe']
    };

    function syncVisualPageHeaderV37(pageKey) {
        const title=document.getElementById('page-title');
        if(!title) return;
        let wrap=title.closest('.topbar-title-wrap-v37');
        if(!wrap){
            wrap=document.createElement('div');
            wrap.className='topbar-title-wrap-v37';
            title.parentNode.insertBefore(wrap,title);
            wrap.appendChild(title);
            const sub=document.createElement('span');
            sub.id='topbar-context-v37';
            sub.className='topbar-context-v37';
            wrap.appendChild(sub);
        }
        const sub=document.getElementById('topbar-context-v37');
        const meta=visualPageMetaV37[pageKey] || ['', ''];
        if(sub) sub.innerHTML=meta[0] ? `<strong>${meta[0]}</strong><span>${meta[1]}</span>` : '';
    }

    document.querySelectorAll('.nav-item[data-page]').forEach(item=>{
        item.addEventListener('click',()=>syncVisualPageHeaderV37(item.dataset.page));
    });
    const initialPage=document.querySelector('.nav-item.active[data-page]')?.dataset.page || 'dashboard';
    syncVisualPageHeaderV37(initialPage);

    // Microfeedback visual sem alterar o comportamento dos controles existentes.
    document.addEventListener('click',e=>{
        const btn=e.target.closest('button,.btn-primary,.btn-secondary,.btn-danger');
        if(!btn || btn.disabled) return;
        btn.classList.remove('ui-tap-v37');
        void btn.offsetWidth;
        btn.classList.add('ui-tap-v37');
        setTimeout(()=>btn.classList.remove('ui-tap-v37'),260);
    },true);

    // V54 — Backup Administrativo
    function backupSnapshotV54(){const pick=k=>{try{return getStore(k)||[];}catch(_){return [];}};return {format:'FilhosDeAsgardBackup',version:54,generatedAt:new Date().toISOString(),generatedBy:{id:currentUser?.id||'',callsign:currentUser?.callsign||currentUser?.name||'ADMIN'},data:{profiles:pick(DB_USERS),games:pick(DB_GAMES),achievements:pick(DB_ACHIEVEMENTS),achievementAwards:pick(DB_ACHIEVEMENT_AWARDS),achievementProgress:pick(DB_ACHIEVEMENT_PROGRESS),announcements:pick(DB_ANNOUNCEMENTS),announcementReads:Array.isArray(announcementReadsV48)?announcementReadsV48:[],products:pick(DB_PRODUCTS),orders:pick(DB_ORDERS),contributions:pick(DB_CONTRIBUTIONS),activities:pick(DB_ACTIVITIES)}};}
    function refreshBackupPanelV54(){if(!$('backup-users-v54'))return;$('backup-users-v54').textContent=(getStore(DB_USERS)||[]).length;$('backup-games-v54').textContent=(getStore(DB_GAMES)||[]).length;$('backup-orders-v54').textContent=(getStore(DB_ORDERS)||[]).length;$('backup-achievements-v54').textContent=(getStore(DB_ACHIEVEMENTS)||[]).length;}
    function createAdminBackupV54(){if(!isAdmin()){showToast('Apenas ADMIN pode gerar backups.','error');return;}try{const blob=new Blob([JSON.stringify(backupSnapshotV54(),null,2)],{type:'application/json;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`filhos-de-asgard-backup-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);if($('backup-status-v54'))$('backup-status-v54').textContent='Backup gerado';addActivity('Backup administrativo exportado');showToast('Backup administrativo gerado.','success');}catch(e){console.error(e);showToast('Não foi possível gerar o backup.','error');}}
    $('admin-create-backup-v54')?.addEventListener('click',createAdminBackupV54);refreshBackupPanelV54();


    // V55 — Registro de Auditoria
    let auditLogsV55=[], auditUnsubV55=null;
    function auditCategoryV55(action=''){
      const a=String(action).toLowerCase();
      if(/jogo|opera|brief|check-in|relatório|loadout/.test(a))return'games';
      if(/pedido|produto|estoque|loja|venda/.test(a))return'store';
      if(/membro|operador|perfil|cargo/.test(a))return'members';
      if(/conquista|insígnia|progresso/.test(a))return'achievements';
      return'system';
    }
    function auditV55(action,category,details={}){
      if(!isAdmin()||!window.AsgardCloud?.writeAuditV55)return;
      window.AsgardCloud.writeAuditV55(action,category||auditCategoryV55(action),details).catch(()=>{});
    }
    function renderAuditV55(){
      const list=$('admin-audit-list'); if(!list)return;
      const cat=$('audit-filter-v55')?.value||'all',term=($('audit-search-v55')?.value||'').trim().toLowerCase();
      const rows=auditLogsV55.filter(x=>(cat==='all'||x.category===cat)&&(!term||`${x.actorCallsign} ${x.action} ${JSON.stringify(x.details||{})}`.toLowerCase().includes(term)));
      list.innerHTML=rows.length?rows.map(x=>`<article class="audit-row-v55"><div class="audit-icon-v55">${x.category==='games'?'🎯':x.category==='store'?'📦':x.category==='members'?'👥':x.category==='achievements'?'🏅':'⚙️'}</div><div><strong>${escapeHtml(x.action||'Ação administrativa')}</strong><span>${escapeHtml(x.actorCallsign||'ADMIN')} • ${x.createdAt?new Date(x.createdAt).toLocaleString('pt-BR'):''}</span>${Object.keys(x.details||{}).length?`<small>${Object.entries(x.details).map(([k,v])=>`${escapeHtml(k)}: ${escapeHtml(String(v))}`).join(' • ')}</small>`:''}</div></article>`).join(''):'<p class="empty-state">Nenhum registro encontrado.</p>';
    }
    function startAuditV55(){
      if(!isAdmin()||auditUnsubV55||!window.AsgardCloud?.watchAuditLogsV55)return;
      auditUnsubV55=window.AsgardCloud.watchAuditLogsV55(rows=>{auditLogsV55=rows;renderAuditV55();});
    }
    $('audit-filter-v55')?.addEventListener('change',renderAuditV55);
    $('audit-search-v55')?.addEventListener('input',renderAuditV55);
    $('audit-refresh-v55')?.addEventListener('click',()=>{if(auditUnsubV55){auditUnsubV55();auditUnsubV55=null;}startAuditV55();});
    startAuditV55();

    // Registra ações administrativas já centralizadas nas versões recentes.
    const originalUpdateOrderStatusV55=updateOrderStatus;
    updateOrderStatus=async function(id,status){const before=(getStore(DB_ORDERS)||[]).find(o=>o.id===id)?.status||'';const result=await originalUpdateOrderStatusV55(id,status);auditV55('Status de pedido alterado','store',{pedido:String(id).slice(0,8),de:before,para:status});return result;};

    const originalCreateBackupV55=typeof createAdminBackupV54==='function'?createAdminBackupV54:null;
    if(originalCreateBackupV55){createAdminBackupV54=function(){const r=originalCreateBackupV55();auditV55('Backup administrativo exportado','system');return r;};}


    // V56: ações transacionais precisam do servidor e são bloqueadas apenas quando totalmente offline.
    document.addEventListener('click',e=>{
        if(navigator.onLine) return;
        const target=e.target.closest('#btn-whatsapp-checkout,[data-game-loadout],#pr-save');
        if(!target) return;
        e.preventDefault();e.stopImmediatePropagation();
        showToast('Esta ação precisa de conexão para validação. Tente novamente quando a rede voltar.','warning');
    },true);


    // V58 — integridade da tela de Jogos: um erro em um documento não deve ocultar os demais.
    window.addEventListener('asgard:sync',event=>{
        if(event.detail?.key!==DB_GAMES || !currentUser) return;
        const rows=getStore(DB_GAMES);
        if(!Array.isArray(rows)){
            console.warn('[V58 Jogos] Snapshot inválido recebido; mantendo interface protegida.');
            return;
        }
        if(activePageName==='games') requestAnimationFrame(()=>refreshGames());
    });
