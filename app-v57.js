...
(function() {
    ...
    // V57: sincroniza uma vez por sessão ADMIN os status do mês atual para os perfis,
    // garantindo que operadores já marcados como "Em Atraso" antes desta versão também sejam bloqueados.
    if(String(currentUser?.role||'').toLowerCase()==='admin' && window.AsgardCloud?.syncCurrentFinancialStatusesV57){
        setTimeout(()=>window.AsgardCloud.syncCurrentFinancialStatusesV57().catch(err=>console.warn('[V57 financeiro]',err?.code||err)),1800);
    }
