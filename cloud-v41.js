...
async async function removeSession() {
  // Flush pending lightweight legacy mutations before leaving when possible.
  [...legacyWriteTimers.keys()].forEach(key => {
    clearTimeout(legacyWriteTimers.get(key));
    flushLegacyWrite(key);
  });
...
function set(key, value) {
  ...
  clearTimeout(legacyWriteTimers.get(key));
  legacyWriteTimers.set(key, setTimeout(() => flushLegacyWrite(key), 120));
}
