...
async function setAchievementStage(achievementId, userId, stageValue, target) {
  ...
  const id=`stage_${aid}_${uid}`;
  await setDoc(doc(db,'achievement_progress',id),{
    id,achievementId:aid,userId:uid,stageOverride:true,stageValue:value,target:max,
    updatedAt:new Date().toISOString(),updatedBy:me.uid
  },{merge:true});
  return {id,stageValue:value,target:max};
}
